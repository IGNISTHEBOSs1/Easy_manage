...
export interface Payment {
  ...
  method_details: PaymentMethodDetails | null
...
export const getStudents = async (): Promise<Student[]> => {
  const { data, error } = await supabase.from('students').select('*, batches(id, name, timing)').order('created_at', { ascending: false })
  if (error) throw error; return data ?? []
}
...
export async function computeStudentRisks(studentIds: string[]): Promise<Map<string, StudentRisk>> {
  ...
  const { data: fees, error } = await supabase.from('fees').select('id, student_id, total_amount, status').in('student_id', studentIds)
  if (error) throw error
  ...
  for (const sid of studentIds) {
    ...
    const risk_label: RiskLabel = riskScore >= 8 ? 'High Risk' : riskScore >= 4 ? 'Warning' : 'Good'
    map.set(sid, { student_id: sid, total_pending: totalPending, late_count: lateCount, risk_score: riskScore, risk_label })
  }
  return map
}
...
export const getFees = async (): Promise<Fee[]> => {
  const { data, error } = await supabase.from('fees').select('*, students(id, name, phone, batch_id, batch_name_legacy)').order('created_at', { ascending: false })
  if (error) throw error; return data ?? []
}
export const getPendingFees = async (): Promise<Fee[]> => {
  const { data, error } = await supabase.from('fees').select('*, students(id, name, phone, batch_id, batch_name_legacy)').in('status', ['pending', 'partial', 'overdue']).order('due_date', { ascending: true })
  if (error) throw error; return data ?? []
}
...
export const recordPayment = async (feeId: string, amount: number, method: PaymentMethod = 'cash', notes?: string): Promise<PaymentResult> => {
  ...
  if (error) throw error; return data[0] as PaymentResult
}
...
export const getActiveLateFeeRule = async (): Promise<LateFeeRule | null> => {
  const { data, error } = await supabase.from('late_fee_rules').select('*').eq('active', true).order('created_at', { ascending: false }).limit(1).maybeSingle()
  if (error) throw error; return data
}
export const applyLateFee = async (feeId: string): Promise<LateFeeResult> => {
  ...
  if (error) throw error; return data[0] as LateFeeResult
}
...
export const generateRecurringFees = async (year: number, month: number): Promise<RecurringFeeResult[]> => {
  ...
  if (error) throw error; return (data ?? []) as RecurringFeeResult[]
}
