...
export default function AttendancePage() {
  ...
  const load = useCallback(async () => {
    ...
    else show(`Students error: ${extractError(sRes.reason)}`, 'error')
    if (bRes.status === 'fulfilled') {
      allBatches = bRes.value; setBatches(bRes.value)
      if (bRes.value.length > 0 && !selectedBatch)
    ...
    setAttendance(map)
    setLoading(false)
  }, [date, show]) // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => { load() }, [load])

  // Auto-select first batch when batches load
  useEffect(() => {
    if (batches.length > 0 && !selectedBatch) setSelectedBatch(batches[0].name)
  ...
  const allPre   = () => { setSaved(false); setAttendance(a => { const u = {...a}; batchStudents.forEach(s => { u[s.id] = 'present' }); return u }) }
  const allAbs   = () => { setSaved(false); setAttendance(a => { const u = {...a}; batchStudents.forEach(s => { u[s.id] = 'absent'  }); return u }) }

  const save = async () => {
    if (!batchStudents.length) return
    setSaving(true)
    try {
      await upsertAttendance(batchStudents.map(s => ({ student_id: s.id, date, status: attendance[s.id] ?? 'present' })))
      setSaved(true); show('Attendance saved ✓')
    } catch (e) { show(`Failed: ${extractError(e)}`, 'error') }
    finally { setSaving(false) }
  }

  const notifyAbsentees = () => {
    const abs = batchStudents.filter(s => attendance[s.id] === 'absent')
    if (!abs.length) { show('No absentees today 🎉', 'info'); return }
    const d = new Date(date).toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })
    abs.forEach((s, i) => setTimeout(() => {
    ...
    show(`Notifying ${abs.length} absentee${abs.length > 1 ? 's' : ''} via WhatsApp`, 'info')
  }
    ...
    <div className="space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {batches.map(b => {
                ...
                return (
                  <button key={b.id} onClick={() => setSelectedBatch(b.name)}
                    className={`py-2.5 px-3 rounded-xl text-sm font-semibold border transition-all text-left ${selectedBatch === b.name ? 'bg-brand-600 text-white border-brand-600' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-50'}`}>
      ...
      {loading ? (
        ...
        <div className="card p-10 text-center">
          ...
          <p className="text-sm font-bold text-slate-700">{students.length === 0 ? 'No students yet' : `No students in ${selectedBatch || 'this batch'}`}</p>
        ...
        <div className="space-y-3">
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Tap a card to mark absent</p>
              ...
              return (
                <button key={s.id} onClick={() => toggle(s.id)}
                  className={`w-full card p-3.5 flex items-center gap-3 text-left transition-all duration-150 active:scale-[0.98] ${
                    isPresent ? 'border-emerald-200 bg-emerald-50/40 hover:bg-emerald-50' : 'border-red-200 bg-red-50/40 hover:bg-red-50'
                  }`}>
                  <div className={`w-10 h-10 rounded-xl font-bold text-sm flex items-center justify-center shrink-0 ${isPresent ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600'}`}>
          ...
          <button onClick={save} disabled={saving || saved}
            className={`w-full py-3 font-semibold ${saved ? 'btn-secondary' : 'btn-primary'}`}>
            {saving ? <span className="spinner" />
              : saved
              ? <><svg className="w-4 h-4 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg> Saved</>
              : <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" /></svg> Save Attendance</>
            }
          </button>
        </div>
      )}
