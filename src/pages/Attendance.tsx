...
// ─── PDF — compact single-page register ──────────────────────────────────────
// Design goals:
//  • 30 students on ONE A4 page (fits ~38 rows at 18px row height)
//  • Real school register look: numbered rows, thin borders, signature column
//  • Stats + header in a tight top section (~60px total)
//  • Print colours forced so green/red badges survive "Save as PDF"
function generateAttendancePDF(
  ...
  const shortDate = new Date(date).toLocaleDateString('en-IN', {
  ...
  // Compact table rows — all students, no cards
  const rows = students.map((s, i) => {
    ...
    <tr style="background:${bg}">
      ...
      <td class="td-phone">${s.phone}</td>
      <td class="td-status">
        <span class="${isPresent ? 'badge-p' : 'badge-a'}">${isPresent ? 'P' : 'A'}</span>
      </td>
      <td class="td-sig"></td>
    </tr>`
  }).join('')

  // Absentee name list — inline, comma-separated, takes almost no space
...
<head>
<meta charset="UTF-8"/>
<title>Attendance — ${batchName} — ${shortDate}</title>
<style>
  /* ── Reset ── */
  *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
  ...
  .brand-icon svg { width:16px; height:16px; fill:none; stroke:#fff; stroke-width:2; }
  .brand-name { font-size: 15px; font-weight: 800; letter-spacing: -0.02em; color: #0f172a; }
  .brand-sub  { font-size: 9px; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.1em; }

  .report-info { text-align: right; }
  .report-title { font-size: 14px; font-weight: 800; color: #0f172a; letter-spacing:-0.01em; }
  .report-date  { font-size: 10px; color: #64748b; margin-top: 1px; }
  ...
  .meta-pill strong { color: #0f172a; font-weight: 700; }
  ...
  .stat-a { background:#fff1f2; border:1px solid #fecdd3; }
  .stat-t { background:#eff6ff; border:1px solid #bfdbfe; }
  .stat-r { background:#fffbeb; border:1px solid #fde68a; }
  .stat-num  { font-size: 20px; font-weight: 800; line-height: 1; }
  .stat-p .stat-num { color: #16a34a; }
  .stat-a .stat-num { color: #dc2626; }
  .stat-t .stat-num { color: #2563eb; }
  .stat-r .stat-num { color: #d97706; }
  .stat-lbl  { font-size: 9px; font-weight: 600; color: #64748b; text-transform: uppercase; letter-spacing: 0.07em; }
  ...
  .bar-label { display:flex; justify-content:space-between; font-size:9px; color:#94a3b8; margin-bottom:3px; }
  ...
  thead th.th-status,
  thead th.th-sig   { text-align: center; }

  /* Compact data rows — 18px effective row height */
  td { border-bottom: 1px solid #f1f5f9; vertical-align: middle; }
  .td-num    { padding: 4px 8px; width: 30px; color: #94a3b8; font-size: 10px; text-align:right; }
  .td-name   { padding: 4px 10px; font-weight: 600; font-size: 11px; color: #0f172a; width: 35%; }
  .td-phone  { padding: 4px 8px; font-family: monospace; font-size: 10px; color: #64748b; width: 22%; }
  .td-status { padding: 4px 8px; text-align: center; width: 60px; }
  .td-sig    { padding: 4px 8px; width: 90px; border-left: 1px dashed #cbd5e1; }

  /* Last row — no bottom border */
  tr:last-child td { border-bottom: none; }

  /* Status badges — tiny, round */
  .badge-p, .badge-a {
  ...
  .absent-line strong { color: #dc2626; }
  ...
  .sig-line  { display: inline-block; min-width: 140px; border-top: 1px solid #334155; padding-top: 4px; font-size: 9px; color: #64748b; }
...
<div class="header">
  <div class="brand">
    ...
    <div>
      <div class="brand-name">CoachPro</div>
...
<div class="meta-strip">
  <div class="meta-pill"><strong>Batch:</strong> ${batchName}${batchTiming ? ` &nbsp;·&nbsp; ${batchTiming}` : ''}</div>
  <div class="meta-pill"><strong>Date:</strong> ${shortDate}</div>
  <div class="meta-pill"><strong>Generated:</strong> ${generatedAt}</div>
</div>
...
<!-- ── Progress bar ── -->
<div class="bar-wrap">
  <div class="bar-label"><span>Attendance Rate</span><span>${pct}%</span></div>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th class="th-sig">Student Signature</th>
      </tr>
    </thead>
    <tbody>${rows}</tbody>
  </table>
...
<div class="absent-line">
  <strong>Absent (${absentCount}):</strong> ${absentNames}
</div>` : `
<div class="absent-line" style="color:#16a34a;">
  ✓ <strong style="color:#16a34a;">Full attendance</strong> — all ${total} students present.
</div>`}

<!-- ── Footer ── -->
<div class="footer">
  <div class="footer-left">
    CoachPro Institute Manager &nbsp;·&nbsp; ${batchName} &nbsp;·&nbsp; ${shortDate}<br/>
    P = Present &nbsp; A = Absent &nbsp;·&nbsp; Generated: ${generatedAt}
  </div>
  <div class="sig-block">
    <div class="sig-line">Teacher / Instructor Signature</div>
  </div>
</div>
...
export default function AttendancePage() {
  ...
  const allPre = () => {
    setSaved(false)
    setAttendance(a => { const u={...a}; batchStudentsRef.current.forEach(s=>{u[s.id]='present'}); return u })
  }

  const allAbs = () => {
    setSaved(false)
    setAttendance(a => { const u={...a}; batchStudentsRef.current.forEach(s=>{u[s.id]='absent'}); return u })
  }
  ...
  const save = async () => {
    ...
    if (!students.length) { show('No students in this batch', 'error'); return }
      ...
      <div className="card p-4 space-y-4">
        <div>
          ...
          <div className="flex gap-2">
            <input className="input flex-1" type="date" value={date}
              max={new Date().toISOString().split('T')[0]}
              onChange={e => setDate(e.target.value)} />
            {!isToday && (
              <button onClick={() => setDate(new Date().toISOString().split('T')[0])} className="btn-secondary shrink-0 text-xs">
                Today
              </button>
            )}
          </div>
        ...
        <div className="space-y-3">
          ...
          <button onClick={save} disabled={saving || saved}
            className={`w-full py-3.5 font-semibold text-sm rounded-xl transition-all ${
              saved ? 'bg-emerald-50 text-emerald-700 border border-emerald-200 cursor-default' : 'btn-primary'
            }`}>
            {saving ? (
          ...
          {saved && (
            <div className="flex items-center justify-between">
              <p className="text-xs text-slate-400">
                Saved · <button onClick={() => setSaved(false)} className="underline hover:text-slate-600">Edit</button>
              </p>
              <button onClick={triggerPDF}
